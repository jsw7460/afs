...
from jax_models import PolicyEncoderAE, TaskEncoder, TransitionDecoder, RewardDecoder, BehaviorDecoder, Model, Params, InfoDict, SumRewardDecoder, PolicyEncoder
...
def policy_task_encoder_update(key:Any, task_encoder: Model, policy_encoder:Model, reward_decoder: Model, transition_decoder: Model,
    ...
    def task_encoder_loss(task_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        pred_next_state = transition_decoder(states, actions, task_latent)
        pred_rewards = reward_decoder(states, actions, task_latent)
        ...
        # kl_loss = jnp.mean(-0.5 * jnp.sum(1 + task_latent_log_std - task_latent_mu ** 2 - jnp.exp(task_latent_log_std), axis=1), axis=0)

        base_dist = tfd.MultivariateNormalDiag(loc=task_latent_mu, scale_diag=jnp.exp(0.5 * task_latent_log_std))

        policy_embedding_loss = -(sum_rewards * jnp.clip(base_dist.log_prob(jax.lax.stop_gradient(jnp.array(policy_latent))), a_max=LOG_STD_MAX * 16, a_min=LOG_STD_MIN * 16)).mean()
        reg_loss = compute_mmd(task_latent)
        l2_reg = sum(l2_loss(w) for w in jax.tree_leaves(task_encoder_params))
        # policy_embedding_loss = (sum_rewards * jnp.sum(jnp.abs(task_latent - jax.lax.stop_gradient(policy_latent)), axis=-1)).mean()
        loss = reconstruction_loss + reg_loss + policy_embedding_loss + 1e-3 * l2_reg
    ...
    def policy_encoder_loss(policy_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        # task_latent_mu, task_latent_log_std = task_encoder(trajectories)
        ...
        pred_actions = behavior_decoder(jnp.reshape(prev_states, (-1, prev_states.shape[-1])), jnp.repeat(policy_latent, repeats=3, axis=0))
        # policy_embedding_loss = (sum_rewards * jnp.sum(jnp.abs(jax.lax.stop_gradient(task_latent) - policy_latent), axis=-1)).mean()
        ...
        # kl_loss = jnp.mean(-0.5 * jnp.sum(1 + policy_latent_log_std - policy_latent_mu ** 2 - jnp.exp(policy_latent_log_std), axis=1), axis=0)
...
def decoder_update(key:Any, task_latent:jnp.ndarray, policy_latent: jnp.ndarray, reward_decoder: Model, transition_decoder: Model, behavior_decoder: Model,
                   states: jnp.ndarray, actions: jnp.ndarray, next_states: jnp.array, rewards: jnp.ndarray,
                   prev_states: jnp.ndarray, prev_actions: jnp.ndarray):

    def reward_loss(reward_decoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        pred_rewards = reward_decoder.apply_fn({'params': reward_decoder_params}, states, actions, task_latent)
    ...
    def transition_loss(transition_decoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        pred_next_states = transition_decoder.apply_fn({'params': transition_decoder_params}, states, actions, task_latent)
    ...
    def behavior_loss(behavior_decoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        pred_actions = behavior_decoder.apply_fn({'params': behavior_decoder_params}, jnp.reshape(prev_states, (-1, prev_states.shape[-1])), jnp.repeat(policy_latent, repeats=3, axis=0))
        ...
        return loss, {'behavior_loss': loss}
...
def _update_pgte(rng:Any, task_encoder: Model, policy_encoder:Model, reward_decoder: Model, transition_decoder: Model, behavior_decoder:Model,
                 ...
                 sum_rewards:jnp.ndarray, prev_states:jnp.ndarray, prev_actions:jnp.ndarray) \
        -> Tuple[Any, Model, Model, Model, Model, Model, InfoDict]:

    rng, task_key, decoder_key = jax.random.split(rng, 3)

    new_task_encoder, new_policy_encoder, task_info = policy_task_encoder_update(task_key, task_encoder, policy_encoder, reward_decoder, transition_decoder, behavior_decoder,
                                                       trajectories, states, actions, next_states, rewards, sum_rewards, prev_states, prev_actions)

    new_reward_decoder, new_transition_decoder, new_behavior_decoder,  decoder_info = \
        decoder_update(decoder_key, task_info['task_latent'], task_info['policy_latent'], reward_decoder, transition_decoder, behavior_decoder, states, actions,
                       next_states, rewards, prev_states, prev_actions)
...
class PGTETrainer(object):
    def __init__(self, datasets, args):
        ...
        prev_states = jnp.zeros((state_size * (args.n_steps - 1)))
        prev_actions = jnp.zeros((action_size * (args.n_steps - 1)))
        # task_id = jnp.zeros(10 ,)

        # Define network
        task_encoder_def = TaskEncoder(net_arch=[256, 256],  latent_dim=args.latent_dim)
        policy_encoder_def = PolicyEncoder(net_arch = [256, 256], latent_dim=args.latent_dim)
        ...
        # create model
        self.key, task_encoder_key, policy_encoder_key, reward_decoder_key, transition_decoder_key, behavior_decoder_key = jax.random.split(self.key, 6)
        ...
        self.transition_decoder = Model.create(transition_decoder_def, inputs=[transition_decoder_key, states, actions, latents],
                                               tx=optax.adam(learning_rate=args.lr))
        self.behavior_decoder = Model.create(behavior_decoder_def, inputs=[behavior_decoder_key, states, latents],
                                             tx=optax.adam(learning_rate=args.lr))
            ...
            for idx, sample in enumerate(tbar):
                ...
                self.key, new_task_encoder, new_policy_encoder, new_reward_decoder, new_transition_decoder, new_behavior_encoder,  info = \
                    _update_pgte(key, self.task_encoder, self.policy_encoder, self.reward_decoder, self.transition_decoder,
                                 ...
                                 next_states.numpy(), sum_rewards.numpy(), prev_states.numpy(), prev_actions.numpy())
