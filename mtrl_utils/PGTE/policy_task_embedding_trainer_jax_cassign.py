...
from jax_models import PolicyEncoder, TaskEncoder, TransitionDecoder, RewardDecoder, BehaviorDecoder, Model, Params, InfoDict, PolicyEncoder, ReturnConditionedTaskEncoder
...
def policy_task_encoder_update(key:Any, task_encoder: Model, policy_encoder:Model, rctask_encoder, reward_decoder: Model, transition_decoder: Model,
    ...
    key, task_key, policy_key, rct_key = jax.random.split(key, 4)
    def task_encoder_loss(task_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        task_latent_mu, task_latent_log_std = task_encoder.apply_fn({'params': task_encoder_params}, trajectories)
        policy_latent_mu, policy_latent_log_std = policy_encoder(jnp.reshape(prev_states, (prev_states.shape[0], -1)), jnp.reshape(prev_actions, (prev_actions.shape[0], -1)))
        rctask_latent_mu, rctask_latent_log_std = rctask_encoder(trajectories, sum_rewards)

        task_latent_log_std = jnp.clip(task_latent_log_std, LOG_STD_MIN, LOG_STD_MAX)
        policy_latent_log_std = jnp.clip(policy_latent_log_std, LOG_STD_MIN, LOG_STD_MAX)
        rctask_latent_log_std = jnp.clip(rctask_latent_log_std, LOG_STD_MIN, LOG_STD_MAX)

        task_latent = task_latent_mu + jax.random.normal(task_key, shape=task_latent_mu.shape) * jnp.exp(0.5 * task_latent_log_std)
        policy_latent = policy_latent_mu + jax.random.normal(policy_key, shape=policy_latent_mu.shape) * jnp.exp(0.5 * policy_latent_log_std)
        rctask_latent = rctask_latent_mu + jax.random.normal(rct_key, shape=rctask_latent_mu.shape) * jnp.exp(0.5 * rctask_latent_log_std)

        task_dist = tfd.MultivariateNormalDiag(loc=task_latent_mu, scale_diag=jnp.exp(0.5 * task_latent_log_std))
        rctask_dist = tfd.MultivariateNormalDiag(loc=rctask_latent_mu, scale_diag=jnp.exp(0.5 * rctask_latent_log_std))

        task_prob = task_dist.log_prob(task_latent)
        rctask_prob = rctask_dist.log_prob(rctask_latent)
    ...
    def policy_encoder_loss(policy_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        policy_latent_mu, policy_latent_log_std = policy_encoder.apply_fn({'params': policy_encoder_params}, jnp.reshape(prev_states, (prev_states.shape[0], -1)), jnp.reshape(prev_actions, (prev_actions.shape[0], -1)))
        policy_latent_log_std = jnp.clip(policy_latent_log_std, LOG_STD_MIN, LOG_STD_MAX)
        policy_latent = policy_latent_mu + jax.random.normal(policy_key, shape=policy_latent_mu.shape) * jnp.exp(
            0.5 * policy_latent_log_std)
        ...
        reg_loss_2 = compute_mmd(policy_latent)

        loss = reconstruction_loss + reg_loss_2 + l2_reg * 1e-3
        return loss, {'policy_reg_loss': reg_loss_2, 'policy_latent': policy_latent}

    def rctask_encoder_loss(rctask_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        rctask_latent_mu, rctask_latent_log_std = rctask_encoder.apply_fn({'params': rctask_encoder_params}, trajectories, sum_rewards)
        ...
        rct_embedding_loss = jnp.sum(jnp.square(rctask_latent - jax.lax.stop_gradient(policy_latent)), axis=-1).mean()
        l2_reg = sum(l2_loss(w) for w in jax.tree_leaves(rctask_encoder_params))

        loss = rct_embedding_loss + l2_reg * 1e-3
        return loss, {'rct_embedding_loss': rct_embedding_loss}
    ...
    new_rct_encoder, rct_info = rctask_encoder.apply_gradient(rctask_encoder_loss)
    return new_task_encoder, new_policy_encoder, {**task_info, **policy_info, **rct_info}
...
def decoder_update(task_latent:jnp.ndarray, policy_latent: jnp.ndarray, reward_decoder: Model, transition_decoder: Model, behavior_decoder: Model,
                   states: jnp.ndarray, actions: jnp.ndarray, next_states: jnp.array, rewards: jnp.ndarray, prev_states: jnp.ndarray,
                   prev_actions: jnp.ndarray, seq: jnp.ndarray):
...
def _update_pgte(rng:Any, task_encoder: Model, policy_encoder:Model, reward_decoder: Model, transition_decoder: Model, behavior_decoder:Model, rctask_encoder:Model,
        ...
        -> Tuple[Any, Model, Model, Model, Model, Model, Model, InfoDict]:
    ...
    new_task_encoder, new_policy_encoder, task_info = policy_task_encoder_update(task_key, task_encoder, policy_encoder, rctask_encoder, reward_decoder, transition_decoder, behavior_decoder,
    ...
    return rng, new_task_encoder, new_policy_encoder, new_reward_decoder, new_transition_decoder, new_behavior_decoder, rctask_encoder, {**task_info, **decoder_info}
...
class PGTETrainer(object):
    def __init__(self, datasets, args):
        ...
        # Define network
        task_encoder_def = TaskEncoder(net_arch=[256, 256],  latent_dim=args.latent_dim)
        policy_encoder_def = PolicyEncoder(net_arch = [256, 256, 256], latent_dim=args.latent_dim)
        rctask_encoder_def = ReturnConditionedTaskEncoder(net_arch = [256, 256, 256], latent_dim=args.latent_dim)
        reward_decoder_def = RewardDecoder(net_arch=[256, 256, 1])
        transition_decoder_def = TransitionDecoder(net_arch=[256, 256, state_size])
        behavior_decoder_def = BehaviorDecoder(net_arch=[256, 256, 256, action_size])

        # create model
        self.key, task_encoder_key, policy_encoder_key, reward_decoder_key, transition_decoder_key, behavior_decoder_key, rct_key = jax.random.split(self.key, 7)
        ...
        self.behavior_decoder = Model.create(behavior_decoder_def, inputs=[behavior_decoder_key, states, latents, seq],
                                             tx=optax.adam(learning_rate=args.lr))
        self.rctask_encoder = Model.create(rctask_encoder_def, inputs=[rct_key, prev_states, prev_actions],
                                                 tx=optax.adam(learning_rate=args.lr))
    ...
    def train(self, epoch):
        for i in range(epoch):
            ...
            for idx, sample in enumerate(tbar):
                ...
                self.key, new_task_encoder, new_policy_encoder, new_reward_decoder, new_transition_decoder, new_behavior_encoder, rctask_encoder, info = \
                    _update_pgte(key, self.task_encoder, self.policy_encoder, self.reward_decoder, self.transition_decoder,
                                 self.behavior_decoder, self.rctask_encoder, traj.numpy(), states.numpy(), actions.numpy(), rewards.numpy(),
                                 next_states.numpy(), sum_rewards, prev_states.numpy(), prev_actions.numpy(), seq)

                self.rctask_encoder = rctask_encoder
    ...
    def save(self, path, seed, num_data):
        ...
        return os.path.join(path, 'policy_task_encoder_{}_seed_{}_{}.jax'.format(self.mode, seed, num_data)), \
               os.path.join(path, 'reward_decoder_{}_seed_{}_{}.jax'.format(self.mode, seed, num_data)),\
               os.path.join(path, 'transition_decoder_{}_seed_{}_{}.jax'.format(self.mode, seed, num_data)),\
