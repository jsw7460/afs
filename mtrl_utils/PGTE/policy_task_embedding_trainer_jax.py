...
from jax_models import PolicyEncoderAE, TaskEncoderAE, TransitionDecoder, RewardDecoder, BehaviorDecoder, Model, Params, InfoDict, SumRewardDecoder, PolicyEncoder, SimpleLinearModel
...
def policy_task_encoder_update(key:Any, task_encoder: Model, policy_encoder:Model, reward_decoder: Model, transition_decoder: Model,
    ...
    def policy_encoder_loss(policy_encoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        pred_actions = behavior_decoder(jnp.reshape(prev_states, (-1, prev_states.shape[-1])), jnp.repeat(policy_latent, repeats=4, axis=0), seq)
...
def decoder_update(key:Any, task_latent:jnp.ndarray, policy_latent: jnp.ndarray, reward_decoder: Model, transition_decoder: Model, behavior_decoder: Model,
                   task_to_policy:Model, states: jnp.ndarray, actions: jnp.ndarray, next_states: jnp.array, rewards: jnp.ndarray, sum_rewards:jnp.ndarray,
    ...
    def behavior_loss(behavior_decoder_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        pred_actions = behavior_decoder.apply_fn({'params': behavior_decoder_params}, jnp.reshape(prev_states, (-1, prev_states.shape[-1])), jnp.repeat(policy_latent, repeats=4, axis=0), seq)
...
def _update_pgte(rng:Any, task_encoder: Model, policy_encoder:Model, reward_decoder: Model, transition_decoder: Model, behavior_decoder:Model, task_to_policy:Model,
    ...
    new_reward_decoder, new_transition_decoder, new_behavior_decoder, decoder_info = \
        decoder_update(decoder_key, task_info['task_latent'], task_info['policy_latent'], reward_decoder, transition_decoder, behavior_decoder, task_to_policy, states, actions,
                       next_states, rewards, sum_rewards, prev_states, prev_actions,  seq)
...
class PGTETrainer(object):
    def __init__(self, datasets, args):
        ...
        seq = jnp.zeros((args.n_steps, ))
        # task_id = jnp.zeros(10 ,)

        # Define network
        task_encoder_def = TaskEncoderAE(net_arch=[256, 256],  latent_dim=args.latent_dim)
        policy_encoder_def = PolicyEncoderAE(net_arch = [256, 256, 256], latent_dim=args.latent_dim)
        ...
        task_to_policy_def = SimpleLinearModel(latent_dim=args.latent_dim)
        ...
        self.behavior_decoder = Model.create(behavior_decoder_def, inputs=[behavior_decoder_key, states, latents, seq],
                                             tx=optax.adam(learning_rate=args.lr))
        self.task_to_policy = Model.create(task_to_policy_def, inputs=[task_to_policy_key, latents],
                                           tx = optax.adam(learning_rate=args.lr))
    ...
    def train(self, epoch):
        for i in range(epoch):
            ...
            for idx, sample in enumerate(tbar):
                ...
                # sum_rewards = np.clip(sum_rewards.numpy() - sum_rewards.numpy().mean(), a_min=0, a_max=1e+10)
                ...
                self.key, new_task_encoder, new_policy_encoder, new_reward_decoder, new_transition_decoder, new_behavior_encoder, info = \
                    _update_pgte(key, self.task_encoder, self.policy_encoder, self.reward_decoder, self.transition_decoder,
                                 self.behavior_decoder, self.task_to_policy, traj.numpy(), states.numpy(), actions.numpy(), rewards.numpy(),
                ...
                # self.task_to_policy = new_task_to_policy
