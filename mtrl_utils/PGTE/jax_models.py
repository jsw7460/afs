...
class ReturnConditionedTaskEncoder(nn.Module):
    net_arch: List[int]
    latent_dim: int
    activation_fn: Callable[[jnp.ndarray], jnp.ndarray] = nn.relu

    @nn.compact
    def __call__(self, trajectories: jnp.ndarray, sum_reward: jnp.ndarray) -> jnp.ndarray:
        inputs = jnp.concatenate([trajectories, sum_reward], -1)

        task_hidden = MLP(net_arch=self.net_arch, activation_fn=self.activation_fn, last_layer_activation=True)(inputs)
